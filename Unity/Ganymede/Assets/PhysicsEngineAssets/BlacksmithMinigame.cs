...
/// <summary>
/// Moves a plain Camera through a sequence of waypoint stations
/// using a quadratic Bezier curve for smooth animated transitions.
/// Creates its own Next/Previous UI buttons at runtime — no canvas setup needed.
///
/// SETUP:
/// 1. Attach this script to your Camera GameObject.
/// 2. Populate the Stations array with your waypoint GameObjects (in order).
/// 3. Press Play — the UI is created automatically.
/// </summary>
public class BlacksmithMinigame : MonoBehaviour
{
    ...
    private void Update()
    {
        // Space or Enter = Next station (fallback if the button doesn't receive clicks)
        if (Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.Return))
            GoToNextStation();

        // Backspace = Previous station (only active when showPreviousButton is on)
    ...
    {
        if (isMoving || index < 0 || index >= stations.Length) return;
    ...
    private void BuildUI()
    {
        ...
        // If there is already one in the scene we leave it alone; otherwise we
        // create one so the buttons work even in a fresh scene.
        ...
        // Create a Screen Space - Overlay canvas owned by this script
        ...
        _stationLabel = CreateLabel(canvasGO,
            "StationLabel",
            "",
            new Vector2(0.5f, 1f),   // anchor top-center
            new Vector2(0f, -40f),   // offset down from top edge
            new Vector2(400f, 50f),
            uiFontSize + 2,
            stationLabelColor);

        // ── Next button (bottom-right) ─────────────────────────────────────────
        _nextButton = CreateButton(canvasGO,
            "NextButton",
            "Next  →",
            new Vector2(1f, 0f),     // anchor bottom-right
            new Vector2(-160f, 60f), // offset from corner
            new Vector2(140f, 50f));

        _nextButton.onClick.AddListener(GoToNextStation);

        // ── Previous button (bottom-left, optional) ───────────────────────────
        if (showPreviousButton)
        {
            _prevButton = CreateButton(canvasGO,
                "PrevButton",
                "←  Back",
                new Vector2(0f, 0f),    // anchor bottom-left
                new Vector2(160f, 60f), // offset from corner
                new Vector2(140f, 50f));
    ...
    /// Creates a TextMeshPro label (or Unity Text fallback).
    private TMP_Text CreateLabel(GameObject parent, string name, string text,
    ...
    /// Creates a styled button with a background image and label.
    ...
    /// <summary>
    /// Returns true when the ingot workflow has completed whatever is needed
    /// at the current station and the user may press Next.
    ///
    /// Station index mapping (matches the order you fill in the Inspector):
    ///   0 = Table   — locked until user picks an ingot (IngotHeld)
    ///   1 = Forge   — always unlocked (bellows are free-form; user presses Next when ready)
    ...
    {
        if (ingotInteraction == null) return true;   // no interaction script — always allow

        switch (currentStationIndex)
        {
            case 0:   // Table: must have picked up an ingot before moving to forge
                return ingotInteraction.Stage == IngotInteraction.WorkflowStage.IngotHeld;

            case 4:   // Table (return): locked while the return animation is playing
                return ingotInteraction.Stage == IngotInteraction.WorkflowStage.WaitingForPliers;

            default:  // Forge, Anvil, Barrel — user decides when they are done
                return true;
        }
    }
    ...
    {
        if (targetIndex == currentStationIndex) return;
    ...
    private IEnumerator MoveAlongBezier(int fromIndex, int toIndex)
    {
        ...
        while (elapsed < travelDuration)
        {
            ...
            transform.position =
                (om * om) * startPos +
                (2f * om * curved) * control +
                (curved * curved) * endPos;
    ...
    /// Drives the IngotInteraction script through its workflow steps.
    private void HandleIngotPlacement(int stationIndex)
    {
        ...
        switch (stationIndex)
        {
            case 1:   // Arrived at Forge
                ingotInteraction.PlaceInForge();
                break;
            case 2:   // Arrived at Anvil
                ingotInteraction.PlaceOnAnvil();
                break;
            case 3:   // Arrived at Barrel
                ingotInteraction.PlaceInBarrel();
                break;
            case 4:   // Arrived back at Table
    ...
    private void OnDrawGizmos()
    {
        ...
        for (int i = 0; i < stations.Length; i++)
        {
            ...
            if (stations[i].waypoint == null || stations[next].waypoint == null) continue;
